...
void video(char *date) {
    char *command[150]; 
    snprintf(command, sizeof(command), "ffmpeg -f video4linux2 -i /dev/video0 -t 10 %s.mp4", date);
    int status = system();
    if (status == -1) {
        printf("Failed to execute command\n");
    } else {
        printf("Video recorded successfully\n");
    }
}
    ...
    if (strcmp(message->topic, ALARM_TRIGGER) == 0){
        ...
        if(data == NULL){
            ...
            if (seconds > COOLDOWN_PERIOD || last_execution == 0) {
                ...
                char *date;
                cJSON *json_item = cJSON_GetObjectItem(data, "date");
                if (json_item != NULL) date = json_item->valuestring;
                // Aquí colocarías la lógica para grabar el vídeo o guardar la información
                video(date);
