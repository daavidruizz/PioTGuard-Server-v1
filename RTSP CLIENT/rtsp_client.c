...
#include "config_struct.h" //cambiar ruta
...
int main (int argc, char *argv[])
{
    ...
    GTlsDatabase* database = G_TLS_DATABASE(g_tls_file_database_new (rtsp_config.rtsp_ca_cert, &error));
    //GTlsDatabase* database = g_tls_file_database_new ("/home/enthusiasticgeek/gstreamer/cert/toyCA.pem", &error);
    if (error != NULL) {
        g_printerr("Error al crear la base de datos de certificados TLS: %s\n", error->message);
        g_clear_error(&error);
    }
    ...
    gst_bin_add_many (GST_BIN (pipeline), rtspsrc, rtph264depay, h264parse, avdec_h264, videoqueue0, videoconvert, video_sink, rtppcmadepay, NULL);
